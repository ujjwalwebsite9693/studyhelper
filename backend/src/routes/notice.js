...
// GET /api/notice/ticker  (no login — powers the scrolling banner on the
// public landing page; only notices the admin has explicitly marked)
router.get('/ticker', async (req, res) => {
  const notices = await Notice.find({ ticker: true }).sort({ createdAt: -1 }).limit(5).select('title message');
  res.json(notices);
});
...
router.post('/', requireAdmin, async (req, res) => {
  const { title, message, important, pinned, targetBranch, targetSemester, ticker } = req.body;
  if (!title || !message) return res.status(400).json({ message: 'Title and message are required' });
  const notice = await Notice.create({
    title, message, important: !!important, pinned: !!pinned, ticker: !!ticker,
    targetBranch: targetBranch || 'ALL',
    targetSemester: targetSemester ? Number(targetSemester) : 0,
  });
  logAdmin('notice.send', `${notice.title} → ${notice.targetBranch}/sem ${notice.targetSemester || 'all'}${notice.ticker ? ' (+ landing ticker)' : ''}`);
  res.status(201).json(notice);
});

// POST /api/notice/:id/ticker  (toggle without deleting/recreating the notice)
router.post('/:id/ticker', requireAdmin, async (req, res) => {
  const notice = await Notice.findById(req.params.id);
  if (!notice) return res.status(404).json({ message: 'Not found' });
  notice.ticker = !notice.ticker;
  await notice.save();
  logAdmin('notice.ticker_toggle', `${notice.title} → ${notice.ticker ? 'on' : 'off'}`);
  res.json(notice);
});
