...
export default function AdminNotices() {
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(true);
    try {
      await adminApi.post('/notice', { title, message, important, pinned, targetBranch, targetSemester, ticker });
      toast.success('Notice sent');
      setTitle(''); setMessage(''); setImportant(false); setPinned(false); setTicker(false); setTargetBranch('ALL'); setTargetSemester(0);
      load();
    } catch (err) {
  ...
  async function toggleTicker(id) {
    try {
      const res = await adminApi.post(`/notice/${id}/ticker`);
      toast.success(res.data.ticker ? 'Added to landing ticker' : 'Removed from landing ticker');
      setNotices((prev) => prev.map((n) => (n._id === id ? { ...n, ticker: res.data.ticker } : n)));
    } catch {
            ...
            <div key={n._id} className={`glass rounded-lg p-3 border-l-4 ${n.important ? 'border-l-amber-400' : 'border-l-brand-500'} flex items-start justify-between gap-3`}>
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {n.ticker && <span className="text-xs">📢</span>}
              ...
              <div className="flex gap-2 shrink-0">
                <button onClick={() => toggleTicker(n._id)} className="text-xs px-3 py-1.5 rounded-lg border border-white/10 hover:bg-white/5">
                  {n.ticker ? 'Remove from ticker' : 'Add to ticker'}
                </button>
                <button onClick={() => handleDelete(n._id)} className="text-xs px-3 py-1.5 rounded-lg border border-red-400/30 text-red-300 hover:bg-red-400/10">Delete</button>
              </div>
            </div>
          ))}
